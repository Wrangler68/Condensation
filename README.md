...
Validation: 17 checks passed natively, in an isolated notebook environment, and in browser WebAssembly. Default outputs exactly match the former split implementation. Browser checks exercise reactive inputs, numerical verification and constrained searches. Original supporting evidence is embedded in the notebook. The old split notebooks, package, data, documentation and test scripts have been consolidated and removed; the three source PDFs are in the repository root.
