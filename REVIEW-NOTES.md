...
- **Page width.** marimo 0.24.2 accepts only `normal`, `compact`, `medium`, `full`, and `columns`. `wide` never starts in WebAssembly: `Invalid enum value 'wide'` at `app_config.width`. `full` is the wide page.
